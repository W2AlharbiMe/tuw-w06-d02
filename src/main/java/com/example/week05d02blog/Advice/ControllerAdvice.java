package com.example.week05d02blog.Advice;
...
public class ControllerAdvice {
    ...
    public ResponseEntity<List<ApiResponse>> MethodArgumentNotValidException(MethodArgumentNotValidException e) {
        ArrayList<ApiResponse> errorResponses = new ArrayList<>();

        for(FieldError fieldError: e.getFieldErrors()) {
            ApiResponse errorResponse = new ApiResponse(fieldError.getDefaultMessage());
    ...
    public ResponseEntity<ApiResponse> DataIntegrityViolationException(DataIntegrityViolationException e){
        ...
        if(e.getMessage().toLowerCase().contains("duplicate entry")) {
            return ResponseEntity.status(400).body(new ApiResponse("duplicated entry."));
        }
